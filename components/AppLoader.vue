<template>
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlns:xlink="http://www.w3.org/1999/xlink"
    style="margin: auto; background: #fff; display: block"
    width="88px"
    height="88px"
    viewBox="0 0 100 100"
    preserveAspectRatio="xMidYMid"
  >
    <g transform="translate(78,50)">
      <g transform="rotate(0)">
        <circle cx="0" cy="0" r="7" fill="#88cc88" fill-opacity="1">
          <animateTransform
            attributeName="transform"
            type="scale"
            begin="-0.3308823529411765s"
            values="1.44 1.44;1 1"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
          ></animateTransform>
          <animate
            attributeName="fill-opacity"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
            values="1;0"
            begin="-0.3308823529411765s"
          ></animate>
        </circle>
      </g>
    </g>
    <g transform="translate(72.65247584249853,66.45798706418924)">
      <g transform="rotate(36)">
        <circle cx="0" cy="0" r="7" fill="#88cc88" fill-opacity="0.9">
          <animateTransform
            attributeName="transform"
            type="scale"
            begin="-0.2941176470588236s"
            values="1.44 1.44;1 1"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
          ></animateTransform>
          <animate
            attributeName="fill-opacity"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
            values="1;0"
            begin="-0.2941176470588236s"
          ></animate>
        </circle>
      </g>
    </g>
    <g transform="translate(58.65247584249853,76.6295824562643)">
      <g transform="rotate(72)">
        <circle cx="0" cy="0" r="7" fill="#88cc88" fill-opacity="0.8">
          <animateTransform
            attributeName="transform"
            type="scale"
            begin="-0.2573529411764706s"
            values="1.44 1.44;1 1"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
          ></animateTransform>
          <animate
            attributeName="fill-opacity"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
            values="1;0"
            begin="-0.2573529411764706s"
          ></animate>
        </circle>
      </g>
    </g>
    <g transform="translate(41.347524157501475,76.6295824562643)">
      <g transform="rotate(108)">
        <circle cx="0" cy="0" r="7" fill="#88cc88" fill-opacity="0.7">
          <animateTransform
            attributeName="transform"
            type="scale"
            begin="-0.22058823529411767s"
            values="1.44 1.44;1 1"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
          ></animateTransform>
          <animate
            attributeName="fill-opacity"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
            values="1;0"
            begin="-0.22058823529411767s"
          ></animate>
        </circle>
      </g>
    </g>
    <g transform="translate(27.347524157501475,66.45798706418925)">
      <g transform="rotate(144)">
        <circle cx="0" cy="0" r="7" fill="#88cc88" fill-opacity="0.6">
          <animateTransform
            attributeName="transform"
            type="scale"
            begin="-0.18382352941176475s"
            values="1.44 1.44;1 1"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
          ></animateTransform>
          <animate
            attributeName="fill-opacity"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
            values="1;0"
            begin="-0.18382352941176475s"
          ></animate>
        </circle>
      </g>
    </g>
    <g transform="translate(22,50)">
      <g transform="rotate(180)">
        <circle cx="0" cy="0" r="7" fill="#88cc88" fill-opacity="0.5">
          <animateTransform
            attributeName="transform"
            type="scale"
            begin="-0.1470588235294118s"
            values="1.44 1.44;1 1"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
          ></animateTransform>
          <animate
            attributeName="fill-opacity"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
            values="1;0"
            begin="-0.1470588235294118s"
          ></animate>
        </circle>
      </g>
    </g>
    <g transform="translate(27.34752415750147,33.54201293581076)">
      <g transform="rotate(216)">
        <circle cx="0" cy="0" r="7" fill="#88cc88" fill-opacity="0.4">
          <animateTransform
            attributeName="transform"
            type="scale"
            begin="-0.11029411764705883s"
            values="1.44 1.44;1 1"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
          ></animateTransform>
          <animate
            attributeName="fill-opacity"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
            values="1;0"
            begin="-0.11029411764705883s"
          ></animate>
        </circle>
      </g>
    </g>
    <g transform="translate(41.34752415750147,23.3704175437357)">
      <g transform="rotate(252)">
        <circle cx="0" cy="0" r="7" fill="#88cc88" fill-opacity="0.3">
          <animateTransform
            attributeName="transform"
            type="scale"
            begin="-0.0735294117647059s"
            values="1.44 1.44;1 1"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
          ></animateTransform>
          <animate
            attributeName="fill-opacity"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
            values="1;0"
            begin="-0.0735294117647059s"
          ></animate>
        </circle>
      </g>
    </g>
    <g transform="translate(58.65247584249852,23.370417543735698)">
      <g transform="rotate(288)">
        <circle cx="0" cy="0" r="7" fill="#88cc88" fill-opacity="0.2">
          <animateTransform
            attributeName="transform"
            type="scale"
            begin="-0.03676470588235295s"
            values="1.44 1.44;1 1"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
          ></animateTransform>
          <animate
            attributeName="fill-opacity"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
            values="1;0"
            begin="-0.03676470588235295s"
          ></animate>
        </circle>
      </g>
    </g>
    <g transform="translate(72.65247584249852,33.542012935810746)">
      <g transform="rotate(324)">
        <circle cx="0" cy="0" r="7" fill="#88cc88" fill-opacity="0.1">
          <animateTransform
            attributeName="transform"
            type="scale"
            begin="0s"
            values="1.44 1.44;1 1"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
          ></animateTransform>
          <animate
            attributeName="fill-opacity"
            keyTimes="0;1"
            dur="0.36764705882352944s"
            repeatCount="indefinite"
            values="1;0"
            begin="0s"
          ></animate>
        </circle>
      </g>
    </g>
  </svg>
</template>
